...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8081/adminpending-requests`)
      .then(response => {
    ...
    axios.get(`http://localhost:8081/adminapproved-requests`)
      .then(response => {
  ...
  axios.get(`http://localhost:8081/adminrejected-requests`)
  .then(response => {
  ...
  const handleAcceptRequest = (request_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        navigate(`/vehicleAllocate/${request_id}/${userId}/${userName}/${college_id}`);
      }
    });
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.put(`http://localhost:8081/update-reject-status/${request_id}`, { status: 'rejected', reason: result.value })
          .then(response => {
            axios.get(`http://localhost:8081/officepending-requests`)
    ...
    <div style={{ backgroundColor: '#B4B4B3', minHeight: '100vh' , overflow: 'hidden'}}>
      ...
      <Container fluid className="mt-3">
        <Row>
          <Col md={3} className="mb-3">
            <div style={{ backgroundColor: '#D8D9DA', padding: '1rem', height: '35rem' }}>
              ...
              <button className="btn btn-success d-flex align-items-center justify-content-between mt-3 w-100" onClick={handleRejectedRequestsClick}>
               ...
               <button className="btn btn-secondary d-flex align-items-center mt-3 w-100" onClick={handleVehicle}>
                 <p className="me-5" style={{fontSize: '1.2rem'}}>Manage Vehicles</p>
              </button>
              <button className="btn btn-secondary d-flex align-items-center mt-3 w-100" onClick={handleDrivers}>
                 <p className="me-5" style={{fontSize: '1.2rem'}}>Manage Drivers</p>
              </button>
          ...
          <Col md={9}>
            <div style={{ backgroundColor: '#D8D9DA', padding: '1rem', height: '35rem' , overflow: 'hidden'}}>
              {currentView === 'pending' && (
                <div>
